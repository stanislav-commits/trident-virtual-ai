...
export class ChatDocumentationService {
  ...
  async prepareDocumentationContext(params: {
    ...
    try {
      ...
      const searchCitationsForQuery = async (
        ...
        const shouldRetryWithoutCategoryScope = (items: ChatCitation[]) =>
          items.length === 0 &&
          Boolean(effectiveDocumentCategories?.length) &&
          !sourceLockDecision.active &&
          Boolean(semanticQuery && this.isProcedureSemanticQuery(semanticQuery));
        ...
        if (role === 'admin' || !shipId) {
          if (effectiveDocumentCategories?.length || scopedManualIds?.length) {
            ...
            if (!shouldRetryWithoutCategoryScope(scopedCitations)) {
              return scopedCitations;
            }

            this.logger.debug(
              `Documentation category-scoped admin retrieval returned no citations for procedure query="${query.replace(/\s+/g, ' ').trim()}"; retrying without category scope`,
            );
        ...
        if (
          effectiveDocumentCategories?.length ||
          scopedManualIds?.length
        ) {
          if (shouldRetryWithoutCategoryScope(result.citations)) {
            this.logger.debug(
              `Documentation category-scoped retrieval returned no citations for ship=${shipId} procedure query="${query.replace(/\s+/g, ' ').trim()}"; retrying without category scope`,
            );
            return (
              await this.contextService.findContextForQuery(
                shipId,
  ...
  private extractExplicitCertificateExpiryTimestamps(text: string): number[] {
    return extractCertificateExpiryTimestamps(text);
  }
