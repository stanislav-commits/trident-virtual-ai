...
import { isStrictTelemetryInventoryListQuery } from '../../common/telemetry-query-intent.utils';
import { IMMUTABLE_CHAT_CORE_SYSTEM_PROMPT } from '../conversation/chat-core-system-prompt.constants';
import {
  ChatQueryPlan,
...
export class LlmService {
  ...
  private buildUserPrompt(context: LLMContext): string {
    ...
    prompt +=
      "Important language rule: Write the entire answer in the same language as the user's latest message. " +
  ...
  private isTelemetryListQuery(query: string): boolean {
    return isStrictTelemetryInventoryListQuery(query);
  }
