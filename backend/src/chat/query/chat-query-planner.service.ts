...
import {
  isCurrentInventoryTelemetryQuery as matchesCurrentInventoryTelemetryQuery,
  isTelemetryInventoryListQuery,
} from '../../common/telemetry-query-intent.utils';
...
export class ChatQueryPlannerService {
  ...
  private isTelemetryListQuery(query: string): boolean {
    ...
    return isTelemetryInventoryListQuery(query);
  }
  ...
  private isCurrentInventoryTelemetryQuery(query: string): boolean {
    return matchesCurrentInventoryTelemetryQuery(query, {
