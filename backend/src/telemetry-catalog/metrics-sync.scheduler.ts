...
export class MetricsSyncScheduler {
  ...
  async handleMetricsSync() {
    ...
    try {
      const result = await this.valuesSync.syncValuesFromInflux();
      this.logger.log(
  ...
  async handleCatalogRescan() {
    ...
    this.isSyncRunning = true;
    try {
      const result = await this.catalogSync.syncCatalogFromInflux();
      this.logger.log(
        `Metrics catalog rescan finished: ships=${result.shipsSynced}, organizations=${result.organizations.join(',')}, definitions=${result.metricsSynced}, buckets=${result.buckets.join(',')}`,
      );
    } catch (error) {
      this.logger.error(
