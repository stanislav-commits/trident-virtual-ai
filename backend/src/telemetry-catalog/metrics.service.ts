...
export class MetricsService {
  ...
  private fallbackCatalogSyncService: MetricsCatalogSyncService | null = null;
  private fallbackValuesSyncService: MetricsValuesSyncService | null = null;

  @Inject(MetricsCatalogSyncService)
  private readonly catalogSyncService?: MetricsCatalogSyncService;

  @Inject(MetricsValuesSyncService)
  private readonly valuesSyncService?: MetricsValuesSyncService;
  ...
  async syncCatalogFromInflux() {
    return this.getCatalogSyncService().syncCatalogFromInflux();
  }

  async syncValuesFromInflux() {
    return this.getValuesSyncService().syncValuesFromInflux();
  }
  ...
  async syncShipMetrics(
    ...
    return this.getCatalogSyncService().syncShipMetrics(
      shipId,
      organizationName,
      options,
    );
  ...
  async enqueueShipMetricsSync(
    ...
    return this.getCatalogSyncService().enqueueShipMetricsSync(
      shipId,
      organizationName,
      options,
    );
  ...
  private getCatalogSyncService(): MetricsCatalogSyncService {
    if (this.catalogSyncService) {
      return this.catalogSyncService;
    }

    if (!this.fallbackCatalogSyncService) {
      this.fallbackCatalogSyncService = new MetricsCatalogSyncService(
        this.prisma,
        this.influxdb,
        this.metricDescriptions,
        this.getValuesSyncService(),
      );
    }

    return this.fallbackCatalogSyncService;
  }

  private getValuesSyncService(): MetricsValuesSyncService {
    if (this.valuesSyncService) {
      return this.valuesSyncService;
    }

    if (!this.fallbackValuesSyncService) {
      this.fallbackValuesSyncService = new MetricsValuesSyncService(
        this.prisma,
        this.influxdb,
      );
    }

    return this.fallbackValuesSyncService;
  }
  ...
  private formatTelemetryLabel(entry: ShipTelemetryEntry): string {
    ...
    const primary =
      ...
      (normalizedLabel && normalizedLabel !== rawLabel
        ? normalizedLabel
        : entry.field || normalizedLabel || entry.key);
  ...
  private isCurrentVesselCoordinateEntry(entry: ShipTelemetryEntry): boolean {
    const exactText = normalizeTelemetryText(
      [getPreferredTelemetryLabel(entry), entry.field].filter(Boolean).join(' '),
    );
  ...
  private scoreTelemetryEntry(
    ...
    const normalizedLabel = normalizeTelemetryText(
  ...
  private determineTelemetryMatchMode(
    ...
    const exactCandidates = [
      first.field,
      getPreferredTelemetryLabel(first),
    ]
      .map((value) => normalizeTelemetryText(value ?? ''))
      .filter(Boolean);
  ...
  private getDedicatedTankDisplayLabel(
    ...
    return humanizeMetricDisplayName(fieldText);
  }
