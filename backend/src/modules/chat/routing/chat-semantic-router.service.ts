...
export class ChatSemanticRouterService {
  ...
  private buildSystemPrompt(): string {
    return [
      ...
      'LANGUAGE IS IRRELEVANT TO ROUTING — CRITICAL: the examples above are written in English only because prompts are authored in English. The user writes in Russian, Ukrainian, Dutch, Italian or anything else. Route on the INTENT you understand, never on whether the wording matches an English example. In particular, ANY request to CREATE / ADD / ASSIGN / SCHEDULE a task or work order, MARK one done, LOG hours or a defect, CLOSE a defect, or START/STOP watching a metric goes to "metrics" in EVERY language — the "pms" route is READ-ONLY and cannot act, so sending a write request there makes the assistant wrongly answer that it is not allowed to create anything.',
