...
export class ChatSemanticRouterService {
  ...
  private buildSystemPrompt(): string {
    return [
      ...
      '- metrics: current/live telemetry, point-in-time readings, trends, ranges, operational measurements, vessel state from sensors. ALSO route to METRICS for all of the following — even if a manual might be involved in the answer, METRICS owns the primary route and may consult manuals via its own internal tools: telemetry-event questions (refueling/bunkering visible as a tank-level jump, engine start, sudden counter step, alarm trip, threshold crossing); voyage / sailing / GPS-track / distance / position questions ("куда плавали / where did we sail / how many voyages / where are we / how far did we go"); fuel and energy consumption questions ("сколько потратили / how much did we consume / fuel per nm / kWh used"); running-hours / utilization / operating-hours questions ("сколько часов работал X / how many hours did Y run"); is-X-normal / is-it-in-spec questions ("в норме ли / is this normal / within spec / overheating"); efficiency / averages-while-state questions ("расход на милю / average kW at anchor / utilization %"); active-alarms / fault-codes / threshold-crossings ("что алармит / are there alarms / when did X exceed Y"); equipment-on-state ("включён ли X / is Y running / when did Z last stop"). ALSO ROUTE TO METRICS any equipment-specific manual lookup (oil change interval / coolant interval / service interval / fault code meaning / part numbers / consumables / rated value / operating range / how to replace X) when the question names a specific brand / model / asset on this vessel — METRICS has the best ship-manual access via its own RAGFlow tool. Only route to DOCUMENTS for formal procedures, regulations, certificates, historical PMS records, or generic ship-operations procedures. The metrics responder has its OWN bridge to ship manuals (RAGFlow), so a question that mixes telemetry + manual stays on METRICS unless it is PURELY a formal procedure / regulation lookup. Sensor-observable events are NOT historical_procedure — only formal PMS/work-order records belong there.',
