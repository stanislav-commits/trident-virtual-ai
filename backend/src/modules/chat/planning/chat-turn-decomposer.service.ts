...
export class ChatTurnDecomposerService {
  ...
  private buildSystemPrompt(): string {
    return [
      ...
      'ACTION CONFIRMATIONS — CRITICAL: when the previous assistant turn PROPOSED an action (e.g. creating a PMS task with stated title/priority/due date) and the latest user turn is an explicit confirmation ("да", "да, создай", "yes, create it", "подтверждаю", "go ahead"), the ask MUST (a) restate the exact action with all its parameters from the assistant\'s proposal and (b) state explicitly that THE USER HAS ALREADY CONFIRMED it — e.g. "Пользователь явно подтвердил: создай задачу в PMS \'Спланировать бункеровку топлива\', приоритет высокий, срок 2026-07-26. Создай её сейчас." Never emit a bare "да" ask and never drop the already-confirmed fact — the executing engine only sees this one standalone ask.',
