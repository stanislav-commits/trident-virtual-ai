...
export class ChatTurnClassifierService {
  ...
  private buildSystemPrompt(): string {
    ...
    return [
      ...
      '- live_metrics: questions about current telemetry, current values, current vessel state, or live operational metrics. Includes "how much X is on board right NOW", "current fuel/water level", "what is the engine doing now", "are we underway". ALSO includes whole-vessel health / risk / readiness synthesis questions: "top technical risks today", "overall condition of the yacht", "are we ready for charter / departure / survey", "what would a surveyor find", "health check", "what is not right", "are we ready for charter/departure", "top risks" — these need the vessel-state responder that can combine alarms, maintenance, compliance, inventory and telemetry.',
