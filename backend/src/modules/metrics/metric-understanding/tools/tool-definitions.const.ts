    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          description: { type: 'string', description: 'WORK INSTRUCTIONS for whoever does the job — this lands in the task form the user reviews, so make it genuinely useful, not a restatement of the title. Give the concrete steps in order, plus what triggered the task, the target values/tolerances, the parts or consumables likely needed, and any safety/isolation step that must come first (shut down, close the fuel valve, lock out). Use the vessel\'s own manuals via lookup_manual_spec when the equipment is known, and keep it to a short numbered list. Do NOT pad it with "created from the chat".' },
