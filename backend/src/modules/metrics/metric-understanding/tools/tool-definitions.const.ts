...
export const TOOL_DEFINITIONS: ChatToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'PROPOSE a task for the vessel\'s task register — e.g. "add a task to top up DEF", "Erik should replace the fuel filter by Friday". This tool NEVER writes anything: it OPENS the task form for the user, prefilled with these details; they review, edit and confirm there (or close it), and that form performs the save. So call it as soon as you know what to propose, even when the user phrased it as a direct order — a direct order is NOT a confirmation. Then reply with ONE short sentence describing what WILL be created (future tense) and STOP: never say the task was created, never ask the user to type "yes"/"да"/"confirm", and never call this tool twice for the same task. Pick the board: "maintenance" = the Maintenance Plan, ONLY for work on the vessel\'s equipment/assets; "general" = the Tasks board for people-directed chores that are not equipment upkeep (cleaning, provisioning, paperwork, drills).',
      parameters: {
        type: 'object',
        properties: {
          ...
          description: { type: 'string', description: 'WORK INSTRUCTIONS for whoever does the job — this lands in the task form the user reviews, so make it genuinely useful, not a restatement of the title. SOURCE THEM FROM THE VESSEL\'S OWN MANUAL FIRST: whenever the equipment is identified, call lookup_manual_spec for the actual procedure (and for intervals, torques, quantities, part numbers) BEFORE writing this field, and follow what the manual says, naming it briefly ("per the MASE VS-350 manual"). Only fall back to general marine-engineering practice when the manual returns nothing, and say so in one short line so the crew knows the steps are generic. Either way: ordered numbered steps, what triggered the task, target values/tolerances, parts or consumables needed, and the safety/isolation step first (shut down, close the fuel valve, lock out). Keep it short. Do NOT pad it with "created from the chat".' },
