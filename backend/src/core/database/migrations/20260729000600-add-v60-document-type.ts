...
/**
 * v60's two orthogonal axes on the catalogue: what a document IS
 ...
 *
 * `v60_ref` records which workbook row a catalogue row was matched to, because
 * the mapping is NOT by code: v60 renumbered the block, so its 1.6 is our 1.7
 ...
 */
export class AddV60DocumentType20260729000600 implements MigrationInterface {
  name = 'AddV60DocumentType20260729000600';
    ...
    for (const table of ['compliance_doc_master', 'compliance_doc_types']) {
      await queryRunner.query(`
        ALTER TABLE "${table}"
          ...
          ADD COLUMN IF NOT EXISTS "v60_ref" varchar(12)
      `);
    }
  }
